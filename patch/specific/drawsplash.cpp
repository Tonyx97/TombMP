...
void S_DrawSplashes()
{
	...
	for (int i = 0; i < MAX_SPLASHES; ++sptr, ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			auto nSprite = (j == 2 || (j == 0 && (sptr->flags & SPL_RIPPLEINNER)) || (j == 1 && (sptr->flags & SPL_RIPPLEMIDDLE)))
							? ((PHDSPRITESTRUCT*)objects[EXPLOSION1].mesh_ptr) + 4 + ((wibble >> 4) & 3)
							: ((PHDSPRITESTRUCT*)objects[EXPLOSION1].mesh_ptr) + 8;
	...
	for (int i = 0; i < MAX_RIPPLES; ++rptr, ++i)
	{
		...
		auto nSprite = ((PHDSPRITESTRUCT*)objects[EXPLOSION1].mesh_ptr) + 9;
		...
		if (rptr->flags & SPL_MORETRANS)
		{
			...
			{
				nSprite = (PHDSPRITESTRUCT*)objects[EXPLOSION1].mesh_ptr;
