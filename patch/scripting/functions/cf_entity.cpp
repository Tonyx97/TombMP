...
void cf_entity::register_functions(sol::state* vm)
{
	...
	vm->set_function("getItemFromEntity", [&](game_entity* entity) -> ITEM_INFO*
	{
		if (!entity)
			return nullptr;
	...
	vm->set_function("destroyEntity", [&](game_entity* entity)		{ if (entity) KillItem(entity->get_item_id()); });
	vm->set_function("destroyEntityFromItem", [&](ITEM_INFO* item)
	{
		if (!item)
			return;
