...
size_t aes256::encrypt(const aes_array& key, void* plain, const size_t plain_length, aes_array& encrypted)
{
	return encrypt((uint8_t*)key.data(), plain, plain_length, encrypted);
}

size_t aes256::encrypt(uint8_t* key, void* plain, const size_t plain_length, aes_array& encrypted)
{
	aes256 aes(key);

	aes.encrypt_start(plain_length, encrypted);
	aes.encrypt_continue((const unsigned char*)plain, plain_length, encrypted);
	aes.encrypt_end(encrypted);

	return encrypted.size();
}

size_t aes256::decrypt(const aes_array& key, void* encrypted, const size_t encrypted_length, aes_array& plain)
{
	return decrypt((uint8_t*)key.data(), encrypted, encrypted_length, plain);
}

size_t aes256::decrypt(uint8_t* key, void* encrypted, const size_t encrypted_length, aes_array& plain)
{
	aes256 aes(key);

	aes.decrypt_start(encrypted_length);
	aes.decrypt_continue((const unsigned char*)encrypted, encrypted_length, plain);

	return plain.size();
}

void aes256::encrypt_start(const size_t plain_length, aes_array& encrypted)
{
...
void aes256::encrypt_continue(const unsigned char* plain, const size_t plain_length, aes_array& encrypted)
{
	size_t i = 0;
...
void aes256::check_and_encrypt_buffer(aes_array& encrypted)
{
...
void aes256::encrypt_end(aes_array& encrypted)
{
	if (m_buffer_pos > 0)
	{
...
void aes256::decrypt_start(const size_t encrypted_length)
{
...
void aes256::decrypt_continue(const unsigned char* encrypted, const size_t encrypted_length, aes_array& plain)
{
	size_t i = 0;
...
void aes256::check_and_decrypt_buffer(aes_array& plain)
{
